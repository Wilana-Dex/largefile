...
def generate_download_urls(file_id: str, count: int = 1, skip: int = 0) -> list:
    ...
    # Save captcha image for manual solving (works on headless servers)
    with open("captcha.png", "wb") as _f:
        _f.write(r.content)

    print("\n--- CAPTCHA SAVED ---")
    print("File: captcha.png")
    print("If you're remote, download it or open the captcha_url in your browser:")
    print(f"Captcha URL: {captcha.get('captcha_url')}")
    print("---------------------\n")
    response = input("Enter captcha response (type the text you see in captcha.png or open the captcha_url): ")
    ...
    for url in proxy_urls:
        ...
        while not working_link:
            ...
            if free_r.get('status') == "error":
                if free_r.get("message") == "Invalid captcha code":
                    r = requests.get(captcha["captcha_url"])
                    with open("captcha.png", "wb") as _f:
                        _f.write(r.content)
                    print("\nInvalid captcha code, new captcha saved to captcha.png")
                    response = input(f"Enter captcha response: ")
                    continue
                elif free_r.get("message") == "File not found":
